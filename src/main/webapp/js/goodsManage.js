...
$(document).on("click",".templatemo-delete-btn",function () {
    ...
    swal({
        ...
        function () {
            ...
            $.ajax({
                url: "/xb/admin/goods/delete/" + goodsid,
                type: "DELETE",
                success:function (result) {
                    swal(result.msg, "","success");
                    to_page('/xb',currentPage);
...
function getActivity() {
    $.ajax({
        url: "/xb/admin/activity/showjson",
        type: "post",
        success: function (result) {
            if(result.code==100) {
                $("#activity-id").empty();
...
function build_goods_table(path,result) {
    ...
    $.each(goods, function (index,item) {
        ...
        var detailA = $("<a></a>").addClass("templatemo-link").attr("href","/xb/detail?goodsId="+item.goodsid).append("详情");
        //var detailA = $("<a></a>").addClass("templatemo-link").attr("href","www.baidu.com").append("详情");
...
function build_page_nav(path,result) {
    ...
    if(!result.info.pageInfo.hasPreviousPage) {
        ...
        prePage.click(function () {
            to_page('/xb',result.info.pageInfo.prePage);
        });
    }
    ...
    if(!result.info.pageInfo.hasNextPage) {
        ...
        nextPage.click(function () {
            to_page('/xb',result.info.pageInfo.nextPage);
        });
    }

    lastPage.click(function () {
        to_page('/xb',result.info.pageInfo.lastPage);
    });
